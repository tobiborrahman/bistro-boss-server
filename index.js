...
const verifyJWT = (req, res, next) => {
	...
	jwt.verify(token, process.env.ACCESS_SECRET_PASS, (err, decoded) => {
		if (err) {
			return res
				.status(401)
				.send({ err: true, message: 'unauthorized access' });
...
async function run() {
	try {
		...
		app.post('/jwt', (req, res) => {
			const user = req.body;
			const token = jwt.sign(user, process.env.ACCESS_SECRET_PASS, {
				expiresIn: '1h',
			});
			res.send({ token });
		});
		...
		app.get('/carts', verifyJWT, async (req, res) => {
			...
			if (email !== decodedEmail) {
				return res
					.status(403)
					.send({ err: true, message: 'forbidden access' });
			}
